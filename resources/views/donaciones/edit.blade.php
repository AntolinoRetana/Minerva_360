    ...
    <div class="max-w-3xl mx-auto py-6 sm:px-6 lg:px-8">
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-800">Editar Donación</h2>
                <a href="{{ route('donaciones.index') }}" class="text-gray-600 hover:text-gray-900">
                    ← Volver
                </a>
            </div>

            <form action="{{ route('donaciones.update', $donacion) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <label class="relative flex items-center justify-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:border-indigo-500 hover:bg-indigo-50">
                                <input type="radio" name="metodo_pago" value="Efectivo" {{ old('metodo_pago', $donacion->metodo_pago) == 'Efectivo' ? 'checked' : '' }} class="sr-only" required>
                            ...
                            <label class="relative flex items-center justify-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:border-indigo-500 hover:bg-indigo-50">
                                <input type="radio" name="metodo_pago" value="Transferencia" {{ old('metodo_pago', $donacion->metodo_pago) == 'Transferencia' ? 'checked' : '' }} class="sr-only">
                                <div class="text-center">
                                    <div class="text-2xl mb-1">🏦</div>
                            ...
                            <label class="relative flex items-center justify-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:border-indigo-500 hover:bg-indigo-50">
                                <input type="radio" name="metodo_pago" value="Paypal" {{ old('metodo_pago', $donacion->metodo_pago) == 'Paypal' ? 'checked' : '' }} class="sr-only">
                                <div class="text-center">
                                    <div class="text-2xl mb-1">💳</div>
